...
#
# This version explores the fit of a probit model to test whether it is
# computationally feasible and produces materially different predictions.
#
################################################################################
...
past_pts_list <- c('all')
window_list <- c('4 yr.')
seasonality_list <- c('mnwk')
age_int_list <- c('with')
pts_target_list <- c('all')
pts_target_label_list <- c('all')
pts_target_caption_list <- c('all offences')
...
sec_head_list <- c('Table 9: Logit and Probit regressions for all offences')
...
tab_caption_list_all <- sprintf('%s (%s drivers)',
                                tab_sec_head_list,
                                tolower(sex_list))
tab_caption_list_pts <- sprintf('%s (%s, %s drivers)',
                                tab_sec_head_list,
...
tab_path <- sprintf('%s/SAAQ_App_Probit_Tabs_%d.tex', tab_dir, tab_version)
...
cat(sprintf('%% Appendix: Tables of Estimates for Logit and Probit Models\n\n'),
...
for (estn_num in 1:nrow(model_list)) {
  ...
  for (reg_num in 1:length(reg_list)) {
    ...
    } else if (reg_type == 'Probit') {

      # Estimate probit regression model.
      probit_model_1 <- glm(data = saaq_data[sel_obs, ], weights = num,
                         formula = fmla,
                         family = binomial(link = "probit"))
      summ_model <- summary(probit_model_1)

      est_coefs_probit <- summ_model$coefficients
    ...
    if (reg_num == length(reg_list)) {
      ...
      # Bind tables from logistic and LPProbit.
      reg_table_est <- data.frame(est_coefs_log[, c('Estimate', 'Std. Error', 'z value')],
                                  est_coefs_probit[, c('Estimate', 'Std. Error', 'z value')])
      # Change column names.
      colnames(reg_table_est) <- c('Est-Log', 'SE-Log', 'z-Log',
                                   'Est-Prob', 'SE-Prob', 'z-Prob')

      # Multiply LPM estimates by 100,000.
      # reg_table_est[, c('Est-LPM', 'SE-LPM')] <- 100000*
      #   reg_table_est[, c('Est-LPM', 'SE-LPM')]

      # Take ratio of coefficients for comparison.
      # reg_table_est[, 'Est-Ratio'] <- reg_table_est[, 'Est-Log'] /
      #   reg_table_est[, 'Est-Prob']
