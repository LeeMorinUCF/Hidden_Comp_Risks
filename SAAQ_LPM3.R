...
################################################################################
# Tabulate number of incidents by point value.
################################################################################
...
# Create a two-week window around the policy change.
saaq_data[, 'window_28'] <- saaq_data[, 'dinf'] >= '2008-03-18' &
  saaq_data[, 'dinf'] <= '2008-04-14'

summary(saaq_data[saaq_data[, 'window_28'], 'dinf'])
length(saaq_data[saaq_data[, 'window_28'], 'dinf'])
...
# one-month window (2-weeks each side).
##################################################
...
# Model 1: Linear model for traffic violations
# All violations combined: 2-week window.
# Restricted to curr_pts_grp == 1-3
##################################################
...
# saaq_data[, 'sel_obsn'] <- saaq_data[, 'sex'] == 'M' &
#   saaq_data[, 'window_14'] &
#   # saaq_data[, 'curr_pts_grp'] %in% c('1-3', '4-6')
#   saaq_data[, 'curr_pts_grp'] %in% c('1-3')
saaq_data[, 'sel_obsn'] <- saaq_data[, 'sex'] == 'F' &
  saaq_data[, 'window_14'] &
  saaq_data[, 'curr_pts_grp'] %in% c('1-3')
sel_obs <- saaq_data[, 'sel_obsn']
...
# year before (2007) and after (2009).
##################################################


# Set placebo indicator.
saaq_data[, 'placebo'] <- saaq_data[, 'dinf'] >= '2007-04-01'
# saaq_data[, 'placebo'] <- saaq_data[, 'dinf'] >= '2009-04-01'


# Create a two-week window around the policy change.
# Year before (2007):
# saaq_data[, 'window_placebo'] <- saaq_data[, 'dinf'] >= '2007-03-25' &
#   saaq_data[, 'dinf'] <= '2007-04-07'
# Year after (2009):
# saaq_data[, 'window_placebo'] <- saaq_data[, 'dinf'] >= '2009-03-25' &
#   saaq_data[, 'dinf'] <= '2009-04-07'
...
lm_model_1 <- lm(data = saaq_data[sel_obs, ],
                 ...
                 formula = events ~ age_grp +
                   ...
                   curr_pts_grp + placebo*curr_pts_grp)
...
saaq_data[, 'month'] <- month.abb[month(saaq_data[, 'dinf'])]
table(saaq_data[, 'month'], useNA = 'ifany')

# Reorder levels in chronological order.
class(saaq_data[, 'month'])

month_list <- unique(saaq_data[, 'month'])

saaq_data[, 'month'] <- factor(saaq_data[, 'month'], levels = month_list)
