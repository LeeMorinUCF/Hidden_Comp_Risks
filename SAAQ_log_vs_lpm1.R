...
#
# Logistic regression models of numbers of tickets awarded by the
...
#
# This version includes a number of modifications for a reject and resubmit decision.
# One of these is the investigation of logistic regression models
# and comparison to the predictions of the linear probability models.
#
################################################################################
...
# Checking for negative LPM predictions.
lm_model_1 <- lm(data = saaq_data[sel_obs, ], weights = num, formula = chosen_model)
lpm_neg_check(lm_model_1)

# Compare with logistic regression model.
log_model_1 <- glm(data = saaq_data[sel_obs, ], weights = num, formula = chosen_model,
                  family = 'binomial')
summary(log_model_1)

summary(predict(lm_model_1) - predict(log_model_1, type = 'response'))
